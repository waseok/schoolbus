...
export default function StatisticsView({ month, statistics, rows, students, absences, selectedStudent, onMonthChange, onStudentChange }: {
  ...
  const matchingStudents = useMemo(
    () => normalizedQuery ? students.filter((student) => student.name.includes(normalizedQuery)).slice(0, 10) : [],
  ...
  return <section className="main-panel wide">
    ...
    <div className="student-absence-panel">
      <h3>학생별 미탑승 통계</h3>
      <p>학생 이름을 검색하면 해당 학생의 미탑승일, 사유와 총 횟수를 확인할 수 있습니다.</p>
      <label className="student-absence-search">학생 이름 검색
        <input
          type="search"
          value={studentQuery}
          onChange={(event) => {
      ...
      {normalizedQuery && !selectedStudent && <div className="student-absence-results">
        ...
        >
          ...
          <span>{student.grade}학년 {student.class_name}</span>
        </button>)}
        {matchingStudents.length === 0 && <div className="empty-state"><strong>검색 결과가 없습니다.</strong></div>}
      </div>}
      {selectedStudent && <>
        <strong className="absence-total">{selectedStudent} · 총 {selectedRecords.length}일 미탑승</strong>
        <div className="absence-table">{selectedRecords.map((record, index) => <div key={`${record.date}-${index}`}><strong>{koreanDate(record.date)} · {record.busNumber}호차</strong><small>{record.note}</small></div>)}{selectedRecords.length === 0 && <div className="empty-state"><strong>이 달의 미탑승 기록이 없습니다.</strong></div>}</div>
      </>}
      {!normalizedQuery && !selectedStudent && <div className="empty-state"><strong>학생 이름을 입력하세요.</strong></div>}
    </div>
    <p className="prototype-note">공휴일과 재량휴업일은 모든 차량에 공통으로 포함하며, 차량별 미운행 기록을 추가합니다.</p>
  </section>;
}
