...
export async function POST(request: Request) {
  try {
    ...
    const sheetRows = await readRows(uploadedFile);
    if (!sheetRows) return jsonError("학생등록 시트를 찾을 수 없습니다.");
    const actualHeaders = (sheetRows[0] ?? []).map((value) => String(value ?? "").replace(/^\uFEFF/, "").trim());
