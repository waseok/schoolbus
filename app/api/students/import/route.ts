...
export async function POST(request: Request) {
  ...
  const students = assertDatabase(created, studentError, "학생을 일괄 등록하지 못했습니다.");
  const { error: assignmentError } = await db.from("assignments").insert(students.map((student, index) => ({ student_id: student.id, bus_id: busByNumber.get(rows[index].busNumber), stop_name: rows[index].stopName || null, start_date: startDate, end_date: endDate })));
  if (assignmentError) {
    await db.from("students").delete().in("id", students.map((student) => student.id));
    assertDatabase(null, assignmentError, "학생 차량 배정을 저장하지 못했습니다.");
  }
